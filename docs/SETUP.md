...
PWA 就是一般網頁,筆電瀏覽器打開同一個網址就能用(跟手機共用同一組連線設定跟課程清單)。「課堂筆記」卡片裡打字,存檔會自動歸到目前選的課程,存成 `.txt` 檔進 Drive 對應課程的「筆記」資料夾,索引表跟「瀏覽」也看得到,跟照片、錄音一樣。

- **同一天、同一課程共用一個檔案**:重複按「儲存筆記」是更新同一份筆記,不會愈存愈多零碎檔案。存檔後文字框**不會清空**——這份筆記代表「今天這堂課」,可以整堂課持續打、隨時再存一次
- 切換課程時,筆記框內容會跟著換成「那個課程今天」的草稿(各自獨立,不會互相蓋掉;切回來還在)
- 打字時持續自動存草稿到瀏覽器本機(不用擔心分頁被系統關掉弄丟)
- 跟拍照一樣支援離線佇列,沒網路時先留著,恢復連線自動補傳
- 三顆小工具按鈕,都是純文字標記(不是真的螢光筆或分頁介面,檔案還是普通 .txt,方便之後用任何工具打開都看得懂):
  - **⏱ 時間**:在游標處插入目前時間,例如 `[14:23]`,方便回頭對照「這句話大概是什麼時候記的」
  - **★ 重點**:選取文字後點,會用「【】」框起來;沒選取就在游標處插入空的「【】」讓你填
  - **§ 章節**:跳出視窗問標題,插入一條分隔線 + 標題,方便長筆記回頭找段落
- 這個功能**不是**跟 OneNote/Notability 那種「錄音+筆記時間軸同步、點筆記跳到對應錄音時間點」的功能一樣,單純是「打字筆記自動歸檔 + 幾個純文字小標記」;如果你要那種同步播放功能,直接用 OneNote 或 Notability(兩者的錄音+筆記同步都是免費功能),事後把匯出的筆記檔案手動丟進 Drive 對應課程資料夾,對帳工具就會自動撈進索引表
