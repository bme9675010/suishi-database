...
> 重點觀念:系統只會**自動即時**索引「PWA 拍照上傳」跟「丟進 `_收件夾` 的檔案」兩條路。如果你直接把檔案拖進課程資料夾(例如 `隨時資料庫/某課程/2026/07/照片/`),要靠**對帳**才會補進索引表跟 App 瀏覽 —— 而對帳現在有三種觸發方式:**① 打開 App(或從其他 App 切回來)時背景自動同步、② 每天凌晨 3 點自動排程、③ 編輯器手動跑 `reconcileIndex`**。所以直接在 Drive 新增/刪除檔案後,回到 App 打開一下就對上了,完全不用按任何按鈕。
