...
- 打字時會自動存草稿到瀏覽器本機(不用擔心分頁被系統關掉弄丟),按「儲存筆記」才會真的送出歸檔,送出後草稿清空、文字框清空
- 跟拍照一樣支援離線佇列,沒網路時先留著,恢復連線自動補傳
- 這個功能**不是**跟 OneNote/Notability 那種「錄音+筆記時間軸同步、點筆記跳到對應錄音時間點」的功能一樣,單純是「打字筆記自動歸檔」;如果你要那種同步播放功能,直接用 OneNote 或 Notability(兩者的錄音+筆記同步都是免費功能),事後把匯出的筆記檔案手動丟進 Drive 對應課程資料夾,對帳工具就會自動撈進索引表
