...
| 函式 | 用途 |
|---|---|
| `reconcileIndex` | **對帳**:讓索引表跟 Drive 課程資料夾實際內容完全同步 — 補登你直接拖進 Drive 的檔案、清掉已刪除的列、順手把有檔案的課程資料夾補進課程清單。**你直接在 Drive 新增/刪除檔案後,跑這個就對得上了。** |
...
| `installCleanupTrigger` | 裝上「每天凌晨 3 點自動跑一次 `cleanupDeadIndexRows`」的排程(只需執行一次) |

> 重點觀念:系統只會自動索引「PWA 拍照上傳」跟「丟進 `_收件夾` 的檔案」兩條路。如果你直接把檔案拖進課程資料夾(例如 `隨時資料庫/某課程/2026/07/照片/`),索引表跟 App 瀏覽**不會自動看到**,要跑一次 `reconcileIndex` 才會補上。刪除則會由每日排程自動清掉(或手動跑 `reconcileIndex`/`cleanupDeadIndexRows`)。
