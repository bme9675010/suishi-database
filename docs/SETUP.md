...
2. 執行記錄顯示「已安裝排程」即完成 — 之後每天凌晨 3 點會自動**對帳**一次:補登你直接放進 Drive 的檔案、清掉已刪除(含垃圾桶)的索引列、同步課程清單、核銷「待清理提醒」。等於就算你什麼都不做,系統每天也會自己把索引表跟 Drive 對齊一次。
   - 想立刻對帳、不想等到凌晨的話,只要打開 App(或從其他 App 切回來),它就會在背景自動同步一次(見 C 維護工具)。
...
> 重點觀念:系統只會**自動即時**索引「PWA 拍照上傳」跟「丟進 `_收件夾` 的檔案」兩條路。如果你直接把檔案拖進課程資料夾(例如 `隨時資料庫/某課程/2026/07/照片/`),要靠**對帳**才會補進索引表跟 App 瀏覽 —— 而對帳有四種觸發方式:**① 打開 App(或從其他 App 切回來)時背景自動同步(主力,不用按)、② 每天凌晨 3 點自動排程(後盾)、③ App「連線設定」裡的「立即同步 Drive」按鈕(備援,同一畫面也會顯示上次同步成功/失敗,可用來確認自動同步有沒有正常運作)、④ 編輯器手動跑 `reconcileIndex`**。所以直接在 Drive 新增/刪除檔案後,回到 App 打開一下就對上了;萬一背景同步遇到暫時性問題,再進「連線設定」按「立即同步」即可。
